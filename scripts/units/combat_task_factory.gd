...
##   var tasks: Array = CombatTaskFactory.create_for_squad(squad, pos, units)
...
static func create(task_type: int, position: Vector2, unit_id: StringName, extra_params: Dictionary = {}) -> CombatTask:
	_load_params()
	var params: Dictionary = _build_params(position, extra_params)
...
static func create_for_squad(squad_config: Resource, position: Vector2, units: Array) -> Array:
	_load_params()
	var tasks: Array = []
...
## [param unit_id] 所属单位 ID。
static func create_global_override(task_type: int, position: Vector2, unit_id: StringName) -> CombatTask:
...
static func _build_params(position: Vector2, extra: Dictionary) -> Dictionary:
